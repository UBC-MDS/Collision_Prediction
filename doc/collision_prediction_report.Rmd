...
train_cm <- read_csv("../results/train_confusion_matrix.csv", show_col_types = FALSE)
...
test_scores <- read_csv("../results/test_scores.csv", show_col_types = FALSE)
```

# Summary

We attempted to build a classification model using Logistic Regression, which leverages the national collision database of Canada for 2017 to predict whether a collision would result in a fatality or not. The final model returned an accuracy score of `r round(test_scores$test_score[1], 3)` on the test data, however, it achieved a high recall score: `r round(test_cm$fatal[2]/(test_cm$fatal[2] + test_cm$non_fatal[2]), 3)` but a low precision score: `r round(test_cm$fatal[2]/(test_cm$fatal[2] + test_cm$fatal[1]), 3)`, which is expected because the data set has a large amount of false positives and, therefore, the model cannot accurately predict fatalities. Thus, we recommend reviewing and improving this prediction model to make better predictions.

```{r}
test_cm <- test_cm  %>%
  kbl(caption = "Table 1. Confusion Matrix on the Test data") %>%
...
scores <- score_results %>% 
  ...
  kbl(caption = "Table 2. Scores from the Logistic Regression model before and after optimization") %>%
  kable_styling(c("striped", "hover"), full_width = F)
...
scores_after_FS_updated <- scores_after_FS  %>%
  kbl(caption = "Table 3. Scores from the final Logistic Regression model") %>%
  kable_styling(c("striped", "hover"), full_width = F)
