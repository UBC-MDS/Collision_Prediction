...
## Model
The Logistic Regression algorithm was used to build a classification model to predict whether a motor vehicle collision lead to a fatality or not. All variables included in the original data set, with the exception of "C_YEAR", "C_CASE", "C_SEV" were used to fit the model. The hyperparameter's $C$ and $Gamma$ were chosen using 5-fold cross validation with random search `RandomizedSearchCV`. We also performed feature selection using `OneHotEncoder`, `RFECV`, and `LogisticRegression`. The R and Python programming languages and the following R and Python packages were used to perform the analysis: knitr, docopt, scikit-learn, Pandas, Altair, Vegalite. The code used to perform the analysis and create this report can be found here: <https://github.com/UBC-MDS/Collision_Prediction>.
